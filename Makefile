.PHONY: help setup setup-ui setup-api setup-hooks ui-dev ui-typecheck ui-lint ui-format ui-format-check ui-build api-dev api-mypy api-ruff-check api-ruff-format api-ruff-format-check verify
...
help:
	...
	@echo "  make api-dev            Start backend dev server"
	@echo "  make verify             Run all configured checks"
	@echo "  make ui-typecheck       Run frontend TypeScript check"
	@echo "  make ui-lint            Run frontend ESLint"
	@echo "  make ui-format          Run frontend Prettier write"
	...
	@echo "  make api-ruff-format    Run backend Ruff format write"
...
api-ruff-format:
	uv run --project api ruff format api --config api/pyproject.toml --cache-dir api/.ruff_cache

api-ruff-format-check:
	uv run --project api ruff format --check api --config api/pyproject.toml --cache-dir api/.ruff_cache

verify: ui-format ui-typecheck ui-lint ui-format-check ui-build api-ruff-format api-mypy api-ruff-check api-ruff-format-check
